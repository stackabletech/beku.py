...
class TestRenderFromStream(unittest.TestCase):
    def test_default_test_suite(self):
        ...
        expected = EffectiveTestSuite(
            name="default", test_cases=[TestCase(name="smoke", values={"druid": "24.0.0-stackable0.0.0-dev"})]
        )
        self.assertEqual(expected, ets[0], "No select, patch implicitly selected tests smoke")
    ...
    def test_patch_implicit_select(self):
        ...
        ets = renderer_from_stream(fixture)
        expected = EffectiveTestSuite(
            name="latest", test_cases=[TestCase(name="smoke", values={"druid": "24.0.0-stackable0.0.0-dev"})]
        )
        self.assertEqual(expected, ets[0], "No select, patch implicitly selected tests smoke")
    ...
    def test_patch_ignores_test_that_was_not_selected(self):
        ...
        self.assertEqual(
            ["resources"],
            list(dict.fromkeys([tc.name for tc in ets[0].test_cases])),
    ...
    def test_patch_the_same_dimension_twice(self):
        ...
        expected = EffectiveTestSuite(
            name="two-patches-on-the-same-test",
            test_cases=[TestCase(name="smoke", values={"druid": "24.0.0-stackable0.0.0-dev"})],
        )
        self.assertEqual(expected, ets[0], "Take first and then take last out of that.")
    ...
    def test_patch_the_same_test_twice(self):
        ...
        self.assertEqual(expected, ets[0], "First patch select 24.0.0 and the second has only one value to choose from")
    ...
    def test_patch_str_expression(self):
        ...
        ets = renderer_from_stream(fixture)
        expected = EffectiveTestSuite(
            name="select-27.0.0", test_cases=[TestCase(name="smoke", values={"druid": "27.0.0"})]
        )
                  ...
                  - test: smoke
                    dimensions:
                      - name: druid
                        expr: last""")
        ets = renderer_from_stream(fixture)
        self.assertEqual(3, len(ets[0].test_cases), "The [latest] test suite has 3 test cases.")
        self.assertEqual(
            ["smoke", "resources"],
            list(dict.fromkeys([tc.name for tc in ets[0].test_cases])),
              ...
              - name: latest
                patch:
                  - dimensions:
                      - expr: last""")
        ets = renderer_from_stream(fixture)
        self.assertEqual(2, len(ets[0].test_cases), "The [latest] test suite has 2 test cases.")
        self.assertEqual(
            ["smoke", "resources"],
            list(dict.fromkeys([tc.name for tc in ets[0].test_cases])),
            "Two test definitions were selected.",
        ...
        expected = EffectiveTestSuite(
            ...
            test_cases=[
                TestCase(
                    name="smoke",
                    values={
                        "druid": "26.0.0-stackable0.0.0-dev",
                        "openshift": "true",
